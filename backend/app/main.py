...
def run_script(payload: RunRequest):
    ...
    cmd = [str(PYTHON_BIN), str(script_path)] + list(payload.args)
    completed = subprocess.run(cmd, cwd=str(WORKSPACE_ROOT), capture_output=True, text=True)
    return {
...
def pip_install(payload: InstallRequest):
    ...
    cmd = [str(PYTHON_BIN), "-m", "pip", "install", *payload.packages]
    completed = subprocess.run(cmd, cwd=str(WORKSPACE_ROOT), capture_output=True, text=True)
    return {
        "exit_code": completed.returncode,
        "stdout": completed.stdout,
...
def pip_install_file(payload: dict):
    ensure_workspace()
    requirements_path = payload.get("path", "requirements.txt")
    req_file = resolve_user_path(requirements_path)
...
def project_summary():
    ensure_workspace()
    files = []
    for root, _, file_names in os.walk(WORKSPACE_ROOT):
